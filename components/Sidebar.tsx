...
import { DashboardIcon, PlusIcon, TrashIcon } from "@radix-ui/react-icons";
...
function ChatRow({
  ...
  // Function to clean message content from terminal output formatting
  const cleanMessageContent = (content: string) => {
    // Remove the terminal output markers and HTML content
    if (content.includes("---START---") && content.includes("---END---")) {
      // For cases where there might be text before or after the terminal output
      const beforeTerminal = content.split("---START---")[0].trim();
      const afterTerminal = content.split("---END---")[1]?.trim() || "";
      
      if (beforeTerminal || afterTerminal) {
        return beforeTerminal || afterTerminal;
      }
      
      // If no text before/after, just return a generic message
      return "Generated output";
    }
    
    // Remove other HTML tags that might be in the content
          ...
          <p className="text-sm text-gray-600 truncate flex-1 font-medium">
            {lastMessage ? (
              <>
                ...
                {cleanMessageContent(lastMessage.content.replace(/\\n/g, "\n"))}
              </>
            ) : (
